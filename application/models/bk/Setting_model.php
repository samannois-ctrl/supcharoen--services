<?php
 class Setting_model extends CI_Model
 {
	...
	function updateAddrCorp($data){
        ...
        $sql="UPDATE tbl_insurance_company SET company_name = '".$data['company_name']."' , company_full_name ='".$data['company_full_name']."' , company_addr='".$data['company_addr']."' , company_telephone='".$data['company_telephone']."' , company_logo='".$data['filename']."' , company_tax_no='".$data['company_tax_no']."' WHERE id = '".$data['dataID']."'  ";
    ...
    //----------------------------------
    function listCompany($dataStatus=NULL){
		$txtWhere = '';
		if(isset($dataStatus)){
			$txtWhere = " WHERE company_status = '".$dataStatus."' ";
		}
        $sql = "SELECT * FROM tbl_insurance_company $txtWhere ORDER BY company_order ASC ";
        $resultData=$this->db->query($sql)->result();
        return $resultData;
    ...
    function addCompany($company_name,$company_status){
    ...
    //----------------------------------
        $sql="INSERT INTO `tbl_insurance_company` (`id`, `company_name`, `company_code`, `company_status`, `company_order`, `user_update`) "
        ." VALUES "
        ." ('', '".$company_name."', 'company_code', '".$company_status."', '".$MaxNum."', '".$this->session->userdata('user_id')."') ";
        if($this->db->query($sql)){
            $returnValue['DoInsert']=1;
        }
        $returnValue['sql'] = $sql;
