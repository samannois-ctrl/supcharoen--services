			  ...
			  $sumTotal = $sumTotal + $sumRow; 
				?>
				 ...
				 <td><input type="checkbox" onClick="getValue('carcheck','carcheck_amt<?php echo $data->recieveInsID?>','<?php echo $data->recieveInsID?>',this,'<?php echo $data->insurance_id?>','<?php echo $data->insurance_data_type?>')" <?php checkBox($data->carcheck)?> ></td>
				  ...
				  <td><input type="checkbox" onclick="setDcCost('dc_cost<?php echo $data->recieveInsID?>',this)" <?php checkBox($data->dc_cost_check)?>> 
				</td>
		  		  <td><input name="dc_cost<?php echo $data->recieveInsID?>" type="text" class="form-control form-control-sm autonumber autosum" id="dc_cost<?php echo $data->recieveInsID?>"  value="<?php if($data->dc_cost >0){ echo $data->dc_cost; }?>"  onChange="calculateSums()"  ></td>
		  		  ...
		  		  <?php if($audit['canCheckPayment']=='1'){?> 
				  <td>
					<button type="button" class="btn btn-danger btn-xs" onClick="deleteReciveList('<?php echo $data->vehicle_regis." ".$data->province_name?>','<?php echo $data->recieveInsID?>')" >x	</button>
