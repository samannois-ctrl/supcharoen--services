...
<?php //print_r($getData['sql'])."<hr>";?>
<?php  //print_r($getData['sqlSelectData'])."<hr>";?>
...
<table class="table-bordered" width="100%" cellpadding="5">
	...
	<?php foreach($getData['dataPaymentCash'] AS $cashData){ 
			if($cashData->pay_cash_status=='1'){
				$txtClass = "text-primary";	
				$cash_is_pay = $cashData->payment_amount;
				$cash_not_pay=0;
			}else{
				$cash_is_pay = 0;
				$txtClass = "text-danger";
				$cash_not_pay = $cashData->payment_amount;
			}
	...
	<tr>
	   ...
	   <td><?php //print_r($cashData);
			  ...
			  echo $cashData->cust_name?> <!--[ Cash <?php echo $cashData->pay_cash_status?> ]-->
	       </a>												
		  </td>
	   ...
	   <td><?php if($cashData->pay_cash_status=='1'){ echo "ชำระเรียบร้อยแล้ว"; }else if($cashData->pay_cash_status=='0'){ echo "ไม่ระบุ"; }else{ echo "เงินสดค้างชำระ"; } ?></td>
	   <?php for($i=1;$i<=$getData['maxPeriod'];$i++){?>
		<td class="<?php echo $txtClass?>" align="right">
				  <?php 
		...
		<?php }?>
	   <td align="right"><?php   echo number_format($cash_is_pay,2);$sumPay=$sumPay+$cash_is_pay;?></td>
	   <td align="right" class="text-danger"><?php echo number_format($cash_not_pay,2); $sumOverdue = $sumOverdue+$cash_not_pay;?>&nbsp;</td>
	   </tr>
	<?php $n++; } ?>
    <?php }?>
