...
<table style="" class="table ">
   <?php $checkMainID='x'; 
	...
	foreach($resultData['getDb'] AS $data){ 
	      ...
	      $sumRow[$data->mainID] = $data->carcheck_amt+$data->tax_amt+$data->tax_service_amt+$data->act_amt+$data->insurance_amt+$data->dc_cost;
		  if($data->recieve_normal=='1'){ 
			$sumRow[$data->mainID]=$sumRow[$data->mainID]+$data->transfer_amt; 
			if($data->CountMaxRow==''){ $data->CountMaxRow=1;}
		}
	...
	?> 
	<tr class="TopBorder">
	  <td colspan="9"><!--[<?php echo 'checkMainID>'.$checkMainID." mainID>".$data->mainID?>]-->
...
<br>
    ...
    <tr>
	  ...
	  <td><?php echo displaycurrency($data->dc_cost);?></td>
    ...
    <tr>
	  ...
	  echo displaycurrency($sumTotal[$data->mainID]);?></strong></td>
    </tr>
