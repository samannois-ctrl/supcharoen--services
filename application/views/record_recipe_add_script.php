<script>
	...
	function updateRecieveData(changeValue, field) {
		var RecieveID = $('#RecieveID').val();
		var isSeCret = $('#isSeCret').val();
		$.post('<?php echo base_url('Record_recipe/updateRecieveData') ?>', {
			changeValue: changeValue,
			field: field,
			RecieveID: RecieveID,isSeCret:isSeCret
		}, function(data) {
			console.log(data);
	...
	function updateRecieveDetail(reCieveDetailID, insurance_id, ins_data_type) {
		...
		var dc_cost = $('#dc_cost'+ reCieveDetailID).val();
		...
		if ((installment_period == '0') && (installment_amt != '')) {
			...
			var inputArray = [carcheck_amt, tax_amt, tax_service_amt, act_amt, insurance_amt, installment_period, installment_amt, RecieveID, date_recieve, insurance_id, pay_type, bank_id, reCieveDetailID, ins_data_type,dc_cost];
	...
	//--------------------------------
	function  setDcCost(inputname , iam){
		if(iam.checked==false){
			$('#'+inputname).val('');
		}else{
