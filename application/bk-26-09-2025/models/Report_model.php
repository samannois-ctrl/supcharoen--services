		...
		//echo "<br>";
		   ...
		   if(in_array("'Home'", $param['insType'])) {  
			 ...
			 $sqlHome="SELECT 5 AS Seq , 
						...
						'3' AS insurance_type,  
						'travel' AS insurance_type_id ,
						'ประกันบ้าน' AS insurance_type_name ,
						t6.company_name, 
						t6.id AS company_id,  
...
ORDER BY company_id;
	...
	//------------------------------------------------------- insurance_no   hideSuccess
	function list_overdue($data){
		$returnValue = array();
		$txtWhereMonth = "";
		$txtWhereMonthCash = "";
		//--------config------
		 if($data['readConfig']=='yes'){
			  $sql="SELECT * FROM tbl_report_overdue_conf  ";
				 $resultData = $this->db->query($sql)->result();
				 foreach($resultData AS $dataCofig){
					 $ouverDueConfig = $dataCofig->hidesuccess;
				 }
			 if($ouverDueConfig=='1'){
				 $data['hideSuccess']='1';
			 }else if($ouverDueConfig=='0'){
				 $data['hideSuccess']='0';
			 }
		 }
		$txtPaySuccess ='';
		...
		$sql="SELECT
				...
				COALESCE((SELECT SUM(amount) FROM tbl_insurance_payment AS f WHERE is_payment='1' AND f.work_id=a.work_id  ),0) AS sumPay  
			FROM
			...
			WHERE 1 $txtWhereMonth $txtWhereYear AND b.data_status='1'
			GROUP BY
			a.work_id  $txtPaySuccess";
		$returnValue['sql']=$sql;
		...
		$sqlSelectData = "SELECT work_id , period ,
			...
			$returnValue['sqlSelectData']=$sqlSelectData;
		...
		$sql="SELECT
			...
			LEFT JOIN tbl_province AS c ON b.province_regis = c.id 
			LEFT JOIN tbl_agent_data AS d ON b.agent_id = d.id  "
			." WHERE b.pay_cash_status='2'  $txtWhereMonthCash $txtWhereYearCash  ";
