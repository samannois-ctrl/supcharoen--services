 ...
 class Receive_model extends CI_Model
 {
  ...
  #----------------------------------------------------- 
	 function updateRecieveData($data){
		 ...
		 $returnValue['doDb']='0';
		 if($data['RecieveID']==0){
			  $sql="INSERT INTO tb_record_money_received(id , ".$data['field'].") VALUES ('', '".$data['changeValue']."')";
			  if($this->db->query($sql)){
				  $returnValue['doDb']='1';
  ...
  #---------------------------------ๅ-------------------- reCieveID  installment_period,installment_amt
	 function updateRecieveDetail($data){
		 ...
		 $sql="UPDATE tb_record_money_received_detail "
			 ...
			 .", act_amt='".$data['act_amt']."' , insurance='".$data['insurance']."' , insurance_amt='".$data['insurance_amt']."' , installment_period='".$data['installment_period']."', installment_amt='".$data['installment_amt']."'  "
